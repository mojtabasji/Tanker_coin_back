...
export default class Game {
    ...
    // get game data by id
    static async getGameDataById(gameDataId: number): Promise<GameData | null> {
    ...
    static async createGameData(gameData: GameData): Promise<GameData> {
        const query = `INSERT INTO game (mineable, coins, power, activeTanker, unlockedTankers, lastUpdate) VALUES (${gameData.mineable}, ${gameData.coins}, ${gameData.power}, ${gameData.activeTanker.id}, ${gameData.unlockedTankers}, ${gameData.lastUpdate})`;
    ...
    static async updateGameData(gameData: GameData): Promise<GameData> {
        const query = `UPDATE game SET mineable = ${gameData.mineable}, coins = ${gameData.coins}, power = ${gameData.power}, activeTanker = ${gameData.activeTanker.id}, unlockedTankers = ${gameData.unlockedTankers}, lastUpdate = ${gameData.lastUpdate} WHERE id = ${gameData.id}`;
        await database.query(query);
        return gameData;
    }
}